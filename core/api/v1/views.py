...
@extend_schema(
    ...
    parameters=[
        ...
        OpenApiParameter(
            ...
            required=False,
            default="1980-01-01",
            description="Earliest start date of report submission until today, in YYYY-MM-DD format. If used alongside end_date, it will return all the reports in a date range.",
        ),
        OpenApiParameter(
            "end_date",
            OpenApiTypes.DATE,
            required=False,
            default="3000-01-01",
            description="Latest end date of report submission, in YYYY-MM-DD format. If used alongside start_date, it will return all the reports in a date range.",
        ),
        OpenApiParameter(
            "summary_name", str, required=False, description="Name of the report."
        ),
    ],
)
@extend_schema(
...
def full_report_view(request):
    if request.method == "POST":
        ...
        return Response(
            {"detail": "Report saved successfully."}, status=status.HTTP_201_CREATED
        )
...
def concept_detail_view(request, concept_id):
    ...
    if row is None:
        return Response(
            {"error": "Concept not found"}, status=status.HTTP_404_NOT_FOUND
        )
