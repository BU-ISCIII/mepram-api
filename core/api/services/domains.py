...
def list_domains():
    ...
    return {
        "total_medical_concepts": all_domains.aggregate(
...
def domain_concepts(domain_id, query=None, limit=100, offset=0):
    ...
    participant_rows = db.rows(
        models.EventsLong.objects.filter(
    ...
    total = models.EventsLong.objects.filter(
        ...
        total_participants=Count("person_id", distinct=True),
        total_concepts=Count("person_id", distinct=False),
    )
    return {
